...
{
	$commandPackageLogs = ".\ExDC_Scripts\Core_Package_Logs.ps1 -RunLocation `"$location`""
	Invoke-Expression $commandPackageLogs
}
...
$handler_Submenu_Tests_CheckAll=
{
	...
	Enable-AllFunctionsDc

	# Server Functions - Exchange Servers
	Enable-AllFunctionsEx

	# Server Functions - Cluster Nodes
	if (($Exchange2007Powershell -eq $true) -or ($Exchange2003orEarlier -eq $true) -or ($NoEMS -eq $true))
	{
		Enable-AllFunctionsCluster
	}

	# Exchange Functions - All
	if (($Exchange2007Powershell -eq $true) -or ($Exchange2010Powershell -eq $true))
	{
		Enable-AllFunctionsClientAccess
		Enable-AllFunctionsGlobal
		Enable-AllFunctionsRecipient
		Enable-AllFunctionsTransport
		Enable-AllFunctionsMisc
	}

	# UM is special
	if ((($Exchange2007Powershell -eq $true) -or ($Exchange2010Powershell -eq $true)) -and ($UM -eq $true))
	{
		Enable-AllFunctionsUm
	}
...
{
	# Server Functions - Domain Controllers
	Disable-AllFunctionsDc

	# Server Functions - Exchange Servers
	Disable-AllFunctionsEx

	# Server Functions - Cluster Nodes
	if (($Exchange2007Powershell -eq $true) -or ($Exchange2003orEarlier -eq $true) -or ($NoEMS -eq $true))
	{
		Disable-AllFunctionsCluster
	}

	# Exchange Functions - All
	if (($Exchange2007Powershell -eq $true) -or ($Exchange2010Powershell -eq $true))
	{
		Disable-AllFunctionsClientAccess
		Disable-AllFunctionsGlobal
		Disable-AllFunctionsRecipient
		Disable-AllFunctionsTransport
		Disable-AllFunctionsMisc
	}

	# UM is special
	if ((($Exchange2007Powershell -eq $true) -or ($Exchange2010Powershell -eq $true)) -and ($UM -eq $true))
	{
		Disable-AllFunctionsUm
	}
...
$handler_btn_Step1_DC_Discover=
{
	...
	if ((Test-Path ".\dc.txt") -eq $true)
	{
        ...
        $File_Location = $location.path + "\dc.txt"
...
$handler_btn_Step1_Ex_Discover=
{
	...
	if ((Test-Path ".\exchange.txt") -eq $true)
	{
        ...
        $File_Location = $location.path + "\exchange.txt"
...
#		$strStartJob = Start-Job -FilePath ".\ExDC_Scripts\Core_Build_Nodes.ps1" -argument @($location,$session_0) -Name "Core_Build_Nodes.ps1"
	...
	if ((Test-Path ".\ClusterNodes.txt") -eq $true)
	{
	    ...
	    $File_Location = $location.path + "\ClusterNodes.txt"
...
$handler_btn_Step1_Mailboxes_Discover=
{
    ...
    $File_Location = $location.path + "\mailbox.txt"
	...
	if (($Exchange2007Powershell -eq $true) -or ($Exchange2010Powershell -eq $true))
	{
		...
		If (Get-ExOrgBoxStatus = $true)
		{
			...
			If (Get-ExOrgMbxBoxStatus = $true)
			{
				...
				$File_Location = $location.path + "\CheckedMailbox.txt"
	...
	if ((Test-Path registry::HKey_Classes_Root\Excel.Application\CurVer) -eq $true)
	{
		if ($chk_Step4_DC_Report.checked -eq $true)
		{
			write-host "-- Starting to assemble the DC Spreadsheet"
				$commandDCAssembleExcel = ".\ExDC_Scripts\Core_assemble_dc_Excel.ps1 -RunLocation `"$location`""
				Invoke-Expression $commandDCAssembleExcel
		...
		if ($chk_Step4_Ex_Report.checked -eq $true)
		{
			write-host "-- Starting to assemble the Exchange Server Spreadsheet"
				$commandExchAssembleExcel = ".\ExDC_Scripts\Core_assemble_exch_Excel.ps1 -RunLocation `"$location`""
				Invoke-Expression $commandExchAssembleExcel
		...
		if ($chk_Step4_ExOrg_Report.checked -eq $true)
		{
			write-host "-- Starting to assemble the Exchange Organization Spreadsheet"
				$commandExOrgAssembleExcel = ".\ExDC_Scripts\Core_assemble_exorg_Excel.ps1 -RunLocation `"$location`""
				Invoke-Expression $commandExOrgAssembleExcel
	...
	if ((Test-Path registry::HKey_Classes_Root\Word.Application\CurVer) -eq $true)
	{
		if ($chk_Step4_Exchange_Environment_Doc.checked -eq $true)
		{
			write-host "-- Starting to assemble the Exchange Documentation using Word"
				$commandExOrgAssembleExcel = ".\ExDC_Scripts\Core_Assemble_ExDoc_Word.ps1 -RunLocation `"$location`""
				Invoke-Expression $commandExOrgAssembleExcel
...
Function Enable-AllFunctionsDc
{
	$chk_DC_Win32_Bios.checked = $true
	$chk_DC_Win32_ComputerSystem.checked = $true
	$chk_DC_Win32_LogicalDisk.checked = $true
	$chk_DC_Win32_NetworkAdapter.checked = $true
	$chk_DC_Win32_NetworkAdapterConfig.checked = $true
	$chk_DC_Win32_OperatingSystem.checked = $true
	$chk_DC_Win32_PageFileUsage.checked = $true
	$chk_DC_Win32_PhysicalMemory.checked = $true
	$chk_DC_Win32_Processor.checked = $true
	$chk_DC_Registry_AD.checked = $true
	$chk_DC_Registry_OS.checked = $true
	$chk_DC_Registry_Software.checked = $true
	$chk_DC_MicrosoftDNS_Zone.checked = $true
	$chk_DC_MSAD_DomainController.checked = $true
	$chk_DC_MSAD_ReplNeighbor.checked = $true
}

Function Disable-AllFunctionsDc
{
	$chk_DC_Win32_Bios.checked = $false
	$chk_DC_Win32_ComputerSystem.checked = $false
	$chk_DC_Win32_LogicalDisk.checked = $false
	$chk_DC_Win32_NetworkAdapter.checked = $false
	$chk_DC_Win32_NetworkAdapterConfig.checked = $false
	$chk_DC_Win32_OperatingSystem.checked = $false
	$chk_DC_Win32_PageFileUsage.checked = $false
	$chk_DC_Win32_PhysicalMemory.checked = $false
	$chk_DC_Win32_Processor.checked = $false
	$chk_DC_Registry_AD.checked = $false
	$chk_DC_Registry_OS.checked = $false
	$chk_DC_Registry_Software.checked = $false
	$chk_DC_MicrosoftDNS_Zone.checked = $false
	$chk_DC_MSAD_DomainController.checked = $false
	$chk_DC_MSAD_ReplNeighbor.checked = $false
}

Function Enable-AllFunctionsEx
{
	$chk_Ex_Win32_Bios.checked = $true
	$chk_Ex_Win32_ComputerSystem.checked = $true
	$chk_Ex_Win32_LogicalDisk.checked = $true
	$chk_Ex_Win32_NetworkAdapter.checked = $true
	$chk_Ex_Win32_NetworkAdapterConfig.checked = $true
	$chk_Ex_Win32_OperatingSystem.checked = $true
	$chk_Ex_Win32_PageFileUsage.checked = $true
	$chk_Ex_Win32_PhysicalMemory.checked = $true
	$chk_Ex_Win32_Processor.checked = $true
	$chk_Ex_Registry_Ex.checked = $true
	$chk_Ex_Registry_OS.checked = $true
	$chk_Ex_Registry_Software.checked = $true
}

Function Disable-AllFunctionsEx
{
	$chk_Ex_Win32_Bios.checked = $false
	$chk_Ex_Win32_ComputerSystem.checked = $false
	$chk_Ex_Win32_LogicalDisk.checked = $false
	$chk_Ex_Win32_NetworkAdapter.checked = $false
	$chk_Ex_Win32_NetworkAdapterConfig.checked = $false
	$chk_Ex_Win32_OperatingSystem.checked = $false
	$chk_Ex_Win32_PageFileUsage.checked = $false
	$chk_Ex_Win32_PhysicalMemory.checked = $false
	$chk_Ex_Win32_Processor.checked = $false
	$chk_Ex_Registry_Ex.checked = $false
	$chk_Ex_Registry_OS.checked = $false
	$chk_Ex_Registry_Software.checked = $false
}

Function Enable-AllFunctionsCluster
{
	$chk_Cluster_MSCluster_Node.checked = $true
	$chk_Cluster_MSCluster_Network.checked = $true
	$chk_Cluster_MSCluster_Resource.checked = $true
	$chk_Cluster_MSCluster_ResourceGroup.checked = $true
}

Function Disable-AllFunctionsCluster
{
	$chk_Cluster_MSCluster_Node.checked = $false
	$chk_Cluster_MSCluster_Network.checked = $false
	$chk_Cluster_MSCluster_Resource.checked = $false
	$chk_Cluster_MSCluster_ResourceGroup.checked = $false
}

Function Enable-AllFunctionsClientAccess
{
	$chk_Org_Get_ActiveSyncDevice.Checked = $True
	$chk_Org_Get_ActiveSyncPolicy.Checked = $True
	$chk_Org_Get_ActiveSyncVirtualDirectory.Checked = $True
	$chk_Org_Get_AutodiscoverVirtualDirectory.Checked = $True
	$chk_Org_Get_AvailabilityAddressSpace.Checked = $True
	$chk_Org_Get_ClientAccessArray.Checked = $True
	$chk_Org_Get_ClientAccessServer.Checked = $True
	$chk_Org_Get_ECPVirtualDirectory.Checked = $True
	$chk_Org_Get_OABVirtualDirectory.Checked = $True
	$chk_Org_Get_OutlookAnywhere.Checked = $True
	$chk_Org_Get_OwaMailboxPolicy.Checked = $True
	$chk_Org_Get_OWAVirtualDirectory.Checked = $True
	$chk_Org_Get_PowershellVirtualDirectory.Checked = $True
	$chk_Org_Get_RPCClientAccess.Checked = $True
	$chk_Org_Get_ThrottlingPolicy.Checked = $True
	$chk_Org_Get_WebServicesVirtualDirectory.Checked = $True
}

Function Disable-AllFunctionsClientAccess
{
	$chk_Org_Get_ActiveSyncDevice.Checked = $False
	$chk_Org_Get_ActiveSyncPolicy.Checked = $False
	$chk_Org_Get_ActiveSyncVirtualDirectory.Checked = $False
	$chk_Org_Get_AutodiscoverVirtualDirectory.Checked = $False
	$chk_Org_Get_AvailabilityAddressSpace.Checked = $False
	$chk_Org_Get_ClientAccessArray.Checked = $False
	$chk_Org_Get_ClientAccessServer.Checked = $False
	$chk_Org_Get_ECPVirtualDirectory.Checked = $False
	$chk_Org_Get_OABVirtualDirectory.Checked = $False
	$chk_Org_Get_OutlookAnywhere.Checked = $False
	$chk_Org_Get_OwaMailboxPolicy.Checked = $False
	$chk_Org_Get_OWAVirtualDirectory.Checked = $False
	$chk_Org_Get_PowershellVirtualDirectory.Checked = $False
	$chk_Org_Get_RPCClientAccess.Checked = $False
	$chk_Org_Get_ThrottlingPolicy.Checked = $False
	$chk_Org_Get_WebServicesVirtualDirectory.Checked = $False
}

Function Enable-AllFunctionsGlobal
{
	$chk_Org_Get_AddressBookPolicy.Checked = $True
	$chk_Org_Get_AddressList.Checked = $True
	$chk_Org_Get_DatabaseAvailabilityGroup.Checked = $True
	$chk_Org_Get_DAGNetwork.Checked = $True
	$chk_Org_Get_EmailAddressPolicy.Checked = $True
	$chk_Org_Get_ExchangeCertificate.Checked = $True
	$chk_Org_Get_ExchangeServer.Checked = $True
	$chk_Org_Get_MailboxDatabase.Checked = $True
	$chk_Org_Get_MailboxDatabaseCopyStatus.Checked = $True
	$chk_Org_Get_MailboxServer.Checked = $True
	$chk_Org_Get_OfflineAddressBook.Checked = $True
	$chk_Org_Get_OrgConfig.Checked = $True
	$chk_Org_Get_PublicFolderDatabase.Checked = $True
	$chk_Org_Get_Rbac.Checked = $True
	$chk_Org_Get_RetentionPolicy.Checked = $True
	$chk_Org_Get_RetentionPolicyTag.Checked = $True
	$chk_Org_Get_StorageGroup.Checked = $True
}

Function Disable-AllFunctionsGlobal
{
	$chk_Org_Get_AddressBookPolicy.Checked = $False
	$chk_Org_Get_AddressList.Checked = $False
	$chk_Org_Get_DatabaseAvailabilityGroup.Checked = $False
	$chk_Org_Get_DAGNetwork.Checked = $False
	$chk_Org_Get_EmailAddressPolicy.Checked = $False
	$chk_Org_Get_ExchangeCertificate.Checked = $False
	$chk_Org_Get_ExchangeServer.Checked = $False
	$chk_Org_Get_MailboxDatabase.Checked = $False
	$chk_Org_Get_MailboxDatabaseCopyStatus.Checked = $False
	$chk_Org_Get_MailboxServer.Checked = $False
	$chk_Org_Get_OfflineAddressBook.Checked = $False
	$chk_Org_Get_OrgConfig.Checked = $False
	$chk_Org_Get_PublicFolderDatabase.Checked = $False
	$chk_Org_Get_Rbac.Checked = $False
	$chk_Org_Get_RetentionPolicy.Checked = $False
	$chk_Org_Get_RetentionPolicyTag.Checked = $False
	$chk_Org_Get_StorageGroup.Checked = $False

}

Function Enable-AllFunctionsRecipient
{
	$chk_Org_Get_ADPermission.Checked = $True
	$chk_Org_Get_CalendarProcessing.Checked = $True
	$chk_Org_Get_CASMailbox.Checked = $True
	$chk_Org_Get_DistributionGroup.Checked = $True
	$chk_Org_Get_DynamicDistributionGroup.Checked = $True
	$chk_Org_Get_Mailbox.Checked = $True
	$chk_Org_Get_MailboxFolderStatistics.Checked = $True
	$chk_Org_Get_MailboxPermission.Checked = $True
	$chk_Org_Get_MailboxStatistics.Checked = $True
	$chk_Org_Get_PublicFolder.Checked = $True
	$chk_Org_Get_PublicFolderStatistics.Checked = $True
	$chk_Org_Quota.Checked = $True
}

Function Disable-AllFunctionsRecipient
{
	$chk_Org_Get_ADPermission.Checked = $False
	$chk_Org_Get_CalendarProcessing.Checked = $False
	$chk_Org_Get_CASMailbox.Checked = $False
	$chk_Org_Get_DistributionGroup.Checked = $False
	$chk_Org_Get_DynamicDistributionGroup.Checked = $False
	$chk_Org_Get_Mailbox.Checked = $False
	$chk_Org_Get_MailboxFolderStatistics.Checked = $False
	$chk_Org_Get_MailboxPermission.Checked = $False
	$chk_Org_Get_MailboxStatistics.Checked = $False
	$chk_Org_Get_PublicFolder.Checked = $False
	$chk_Org_Get_PublicFolderStatistics.Checked = $False
	$chk_Org_Quota.Checked = $False
}

Function Enable-AllFunctionsTransport
{
	$chk_Org_Get_AcceptedDomain.Checked = $True
	$chk_Org_Get_AdSite.Checked = $True
	$chk_Org_Get_AdSiteLink.Checked = $True
	$chk_Org_Get_ContentFilterConfig.Checked = $True
	$chk_Org_Get_ReceiveConnector.Checked = $True
	$chk_Org_Get_RemoteDomain.Checked = $True
	$chk_Org_Get_RoutingGroupConnector.Checked = $True
	$chk_Org_Get_SendConnector.Checked = $True
	$chk_Org_Get_TransportConfig.Checked = $True
	$chk_Org_Get_TransportRule.Checked = $True
	$chk_Org_Get_TransportServer.Checked = $True
}

Function Disable-AllFunctionsTransport
{
	$chk_Org_Get_AcceptedDomain.Checked = $False
	$chk_Org_Get_AdSite.Checked = $False
	$chk_Org_Get_AdSiteLink.Checked = $False
	$chk_Org_Get_ContentFilterConfig.Checked = $False
	$chk_Org_Get_ReceiveConnector.Checked = $False
	$chk_Org_Get_RemoteDomain.Checked = $False
	$chk_Org_Get_RoutingGroupConnector.Checked = $False
	$chk_Org_Get_SendConnector.Checked = $False
	$chk_Org_Get_TransportConfig.Checked = $False
	$chk_Org_Get_TransportRule.Checked = $False
	$chk_Org_Get_TransportServer.Checked = $False
}

Function Enable-AllFunctionsUm
{
	$chk_Org_Get_UmAutoAttendant.Checked = $True
	$chk_Org_Get_UmDialPlan.Checked = $True
	$chk_Org_Get_UmIpGateway.Checked = $True
	$chk_Org_Get_UmMailbox.Checked = $True
	#$chk_Org_Get_UmMailboxConfiguration.Checked = $True
	#$chk_Org_Get_UmMailboxPin.Checked = $True
	$chk_Org_Get_UmMailboxPolicy.Checked = $True
	$chk_Org_Get_UmServer.Checked = $True

}

Function Disable-AllFunctionsUm
{
	$chk_Org_Get_UmAutoAttendant.Checked = $False
	$chk_Org_Get_UmDialPlan.Checked = $False
	$chk_Org_Get_UmIpGateway.Checked = $False
	$chk_Org_Get_UmMailbox.Checked = $False
	#$chk_Org_Get_UmMailboxConfiguration.Checked = $False
	#$chk_Org_Get_UmMailboxPin.Checked = $False
	$chk_Org_Get_UmMailboxPolicy.Checked = $False
	$chk_Org_Get_UmServer.Checked = $False
}

Function Enable-AllFunctionsMisc
{
	$chk_Org_Get_AdminGroups.Checked = $True
	$chk_Org_Get_Fsmo.Checked = $True
	$chk_Org_Get_ExchangeServerBuilds.Checked = $True
}

Function Disable-AllFunctionsMisc
{
	$chk_Org_Get_AdminGroups.Checked = $False
	$chk_Org_Get_Fsmo.Checked = $False
